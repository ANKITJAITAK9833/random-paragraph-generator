...
form.addEventListener("submit", function (e) {
  ...
  if (amt < 0 || isNaN(amt) || amt > 9) {
    loremText.innerHTML = `<p class="result">text[getRandomNumber()]</p>`;
  } else {
    let loremTxt = text.slice(0, amt);
    loremTxt = loremTxt
      .map((txt) => {
        return `<p class="result">${txt}</p>`;
      })
      .join("");
    loremText.innerHTML = loremTxt;
  }
});
